...
def WHARTON_enable_course_shopping(inputfile, outputfile):
    canvas = get_canvas(TESTING)

    # Do feel free to target account id 81471, SIS ID WHARTON
    WHARTON_ACCOUNT_ID = 81471
    WHARTON_ACCOUNTS = find_accounts_subaccounts(canvas, WHARTON_ACCOUNT_ID)

    # Exclude sub-account 82603 WEMBA - MBA Program for Executives
            ...
            if canvas_course:
                if not account_id.isnumeric():
                    ...
                    try:
                        ...
                        in_SRS = WH_linked_to_SRS(canvas, canvas_id)
                        if in_SRS == False:
                            print("\t not in SRS")
...
def enable_course_shopping(inputfile, outputfile):
    ...
    SAS_ACCOUNTS = find_accounts_subaccounts(canvas, SAS_ACCOUNT_ID)
    SAS_ACCOUNTS.remove(SAS_ONL_ACCOUNT)
    SAS_ACCOUNTS.remove(ADMIN_ACCOUNT)

    SEAS_ACCOUNT_ID = 99238
    SEAS_ACCOUNTS = find_accounts_subaccounts(canvas, SEAS_ACCOUNT_ID)

    NURS_ACCOUNT_ID = 99239
    NURS_ACCOUNTS = find_accounts_subaccounts(canvas, NURS_ACCOUNT_ID)

    AN_ACCOUNT_ID = 99243
    AN_ACCOUNTS = find_accounts_subaccounts(canvas, AN_ACCOUNT_ID)
...
# by default, use MASTER_FILE for input and DISABLE_FILE for output
